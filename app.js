...
client.on("message", async (msg) => {
    if (!msg.author.bot) {
        ...
        if (msg.content.toLowerCase() === "!myrequests") {
            ...
            if (!viewResponse.length) {
                ...
                for (i = 0; i < viewResponse.length; i++) {
                    userResponse += viewResponse[i].current_quantity + " / " + viewResponse[i].current_quantity + " of " + viewResponse[i].item_name + " " + "collected.\n";
                }
        ...
        if (commandSplit === "!request" && !isNaN(shoppingQuantity)) {
            const setupMessage = await client.channels.get(process.env.BOTCHANNEL).send("[processing]");
            ...
            if (typeof setupMessage !== "undefined") {

                console.log(runRequest);

                let richembed = {
                    title: ":id: `" + setupMessage.id + "` - New Request From: ",
                    description: "\n\n<@" + msg.member.id + ">",
                    color: 5508893,
                ...
                msg.channel.fetchMessages({ around: setupMessage.id, limit: 1 })
                    .then(msg => {
                        const fetchedMsg = msg.first();
                        fetchedMsg.edit(richembed);
                    });
